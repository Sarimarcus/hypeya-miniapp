...
class BundleAnalyzer {
  ...
  async analyze() {
    ...
    await this.analyzeBuildManifest().catch((err) => {
      ...
      console.warn(
        "   Tip: For detailed analysis, run webpack build (remove --turbopack) or use @next/bundle-analyzer.\n"
      );
    });
  ...
  analyzePageSizes() {
    ...
    for (const [page, stats] of sortedPages.slice(0, 10)) {
      ...
      console.log(
        `${page.padEnd(30)} ${stats.formatted.gzipped.padStart(8)} ${bar}`
      );
    }
    console.log();
  }
  ...
  generateRecommendations() {
    ...
    const largePages = Array.from(this.bundleStats.entries()).filter(
      ([key, stats]) => !key.startsWith("chunk:") && stats.gzippedSize > 250000
    );
    ...
    console.log(
      "  - Consider using @next/bundle-analyzer for detailed analysis"
    );
    console.log();
  ...
  checkPerformanceBudget() {
    ...
    for (const budget of budgets) {
      if (totalSize <= budget.limit) {
        console.log(
          `  ${budget.emoji} ${budget.name}: ${this.formatSize(
            totalSize
          )} / ${this.formatSize(budget.limit)}`
        );
        break;
      }
    }
