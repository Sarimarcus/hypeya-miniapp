...
interface ServiceWorkerManager {
  ...
  cacheArticle: (articleData: any) => Promise<void>;
}
...
class ServiceWorkerManagerImpl implements ServiceWorkerManager {
  ...
  async cacheArticle(articleData: any): Promise<void> {
    if (!navigator.serviceWorker.controller) {
...
// Component for service worker update notification
export function ServiceWorkerUpdateNotification() {
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 z-50 bg-blue-600 text-white p-4 rounded-lg shadow-lg flex items-center justify-between">
      ...
      <button
        onClick={update}
        className="ml-4 bg-blue-700 hover:bg-blue-800 px-4 py-2 rounded text-sm font-medium"
      >
        Update
      </button>
    </div>
...
// Component showing offline status
